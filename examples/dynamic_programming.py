...
def bruteforce_MK_solve(mk_func):
    ...
    """
    best_fit = 0.0
    best_bitset = utils.create_bitstring_from_int(0, mk_func.m)
    for i in range(2**mk_func.m):
        bs = utils.create_bitstring_from_int(i, mk_func.m)
...
def dp_solve_MK(mk_func):
    ...
    for i in range(q):
        ...
        for a0 in range(2**(2*k-2)):
            tilde_val = 0
            temp_bs = utils.create_bitstring_from_int(a0, 2*k-2)
            for j in range(k-1):
                current_mk_func = mk_func.fitness_map[start + j]
                temp_bs1 = utils.create_bitstring_from_int(0, k)
                for p in range(k):
...
def dp_solve_adjacentMK_tilde(tilde_funcs, pos, k):
    ...
    if pos == 1:
        ...
        for a1 in range(2**(k-1)):
            for a2 in range(2**(k-1)):
                bsa1 = utils.create_bitstring_from_int(a1, k-1)
                bsa2 = utils.create_bitstring_from_int(a2, k-1)
                tilde0_bs = utils.create_bitstring_from_int(0, 2*k-2)
                tilde1_bs = utils.create_bitstring_from_int(0, 2*k-2)
                for i in range(k-1):
                    tilde0_bs[i] = bsa1[i]
    ...
    for a1 in range(2**(k-1)):
        for a2 in range(2**(k-1)):
            bsa1 = utils.create_bitstring_from_int(a1, k-1)
            bsa2 = utils.create_bitstring_from_int(a2, k-1)
            new_tilde_inp = utils.create_bitstring_from_int(0, 2*k-2)
            for i in range(k-1):
                new_tilde_inp[i] = bsa1[i]
            for i in range(k-1):
                new_tilde_inp[k-1+i] = bsa2[i]
            bs_tilde1 = utils.create_bitstring_from_int(0, 2*k-2)
            bs_tilde2 = utils.create_bitstring_from_int(0, 2*k-2)
            for i in range(k-1):
                bs_tilde1[i] = bsa1[i]
            for i in range(k-1):
            ...
            for b in range(2**(k-1)):
                bsb = utils.create_bitstring_from_int(b, k-1)
                for j in range(k-1):
